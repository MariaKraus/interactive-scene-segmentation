...
def train(image_dir: str, label:str):
    ...
    with open(os.getcwd() + "/label_images.txt", 'r') as file:
    ...
    for ep in range(epochs):
        ...
        for i in range(len(train_data)):
            img = cv2.imread(os.path.join(os.getcwd(), "train", "images", train_data[i]))
        ...
        for k in range(len(val_data)):
            img = cv2.imread(os.path.join(os.getcwd(), "train", "images", val_data[k]))
            image_resized = cv2.resize(img, (100, 100))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--i_dir", type=str, default=os.getcwd() + "/train/images/",
                        help="The directory with the training images")
    parser.add_argument("--label", type=str, default=os.getcwd() + "/label_images.txt",
                        help="The directory with the training labels")

    args = parser.parse_args()

    if not os.path.isdir(args.i_dir):
        print("The given directory: ", args.i_dir, " does not exist.")
        exit(0)
